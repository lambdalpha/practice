...
# orignial variable list and target variable
...
# variables after feature eigineering
...
def train_model(csv_file):
    """
    feature eigineering and then train the model
    """
    ...
    return (gbm, std, anomaly_count, df.shape[0], anomaly_rate)

def test_model(model, csv_file, std=None):
     df = feature_trans2(csv_file, var_list_orig, var_list_ang_orig)    
...
def test_model2(model, csv_file, std=None):
...
def test_models(gbm, indir, std=None):
    df = pd.DataFrame(columns = ["file", "timestamp","anomaly_count", "count", "anomaly_rate", "mse", "std", "wtur_flt_main" ])
    i = 0
    for subdir, dirs, files in os.walk(indir):
        for file in files:
            if file.find(".csv") > 0:
                out = test_model2(gbm, subdir + '/' + file, std)
                df.loc[i] = [file] + list(out)
                i = i + 1
    return df
...
if __name__ == '__main__':
    ...
    #df = test_models(gbm, "/home/grid/data/wind/", std)
    #df.to_csv("/home/grid/data/wind2/out2.csv", index=False)
    df = test_models(gbm, "/home/grid/data/wind/", std)
    df.to_csv("/home/grid/data/wind2/out4.csv", index=False)
